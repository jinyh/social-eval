...
| 模型 | 时间稳定性 | 输出格式 | 综合评价 | 备注 |
|------|-----------|----------|----------|------|
| **GPT-5.4** | ✅ 高（±4分） | ✅ 稳定 | **推荐** | 6个维度中5个稳定 |
| **Gemini 3.1 Pro** | ⚠️ 低（±35分） | ✅ 稳定 | 不推荐 | 评分波动大 |
| **Kimi-K2.6** | ❌ 极低 | ❌ 不稳定 | 禁用 | 输出格式解析失败率 100% |
...
- **不推荐模型**：Gemini 3.1 Pro（波动大，±35分）、Kimi-K2.6（输出不稳定）
- **推荐框架**：v2.8（应用概念操作化定义）
