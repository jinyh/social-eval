...
configs/
  frameworks/      # 各学科知识体系 YAML 配置文件
    ...
    law-v2.6-20260423.yaml  # 法学评价框架 v2.6（研究版，已验证）
    law-v2.8-20260423.yaml  # 法学评价框架 v2.8（推荐，概念操作化）
    law-v2.42-20260507.yaml # 法学评价框架 v2.42（autoresearch 迭代研究分支）
...
| 版本 | 配置文件 | 状态 | 适用场景 | 更新日期 |
|------|----------|------|----------|----------|
| v2.0 | `law-v2.0-20260413.yaml` | 生产稳定版 | 已验证，生产环境使用 | 2026-04-13 |
| v2.6 | `law-v2.6-20260423.yaml` | 研究版 | 已验证，测试达标（std=7.0） | 2026-04-23 |
| v2.8 | `law-v2.8-20260423.yaml` | **推荐** | **概念操作化，现状洞察度改进 41%** | 2026-04-23 |
| v2.42 | `law-v2.42-20260507.yaml` | 研究版 | autoresearch 迭代分支，分析框架建构力优化 | 2026-05-07 |
...
**研究/测试**：v2.6 或 v2.8  
**规程对齐**：v2.43（评分逻辑与 v2.42 严格一致，仅新增 `autonomous_knowledge_signals` 文档性区块）

#### v2.8 核心改进（2026-04-23）

**改进策略**：渐进式改进，只改进现状洞察度维度

**改进内容**：
1. 应用概念操作化定义（详见 `docs/evaluation/concept-operationalization-v1.0.md`）
2. 明确"研究地图"的 3 项必备要素（到达点/未竟点/核心争点）
3. 为每项要素提供搜索关键词和正面/反面示例
4. 量化学术流派覆盖标准（3个及以上→80-100分，2个→60-79分）

**测试结果**：
- 现状洞察度 std：从 15.6 降至 9.2（**-41%**）
- 整体平均 std：9.5 分
- 高置信度比例：16.7%
...
```yaml
框架: v2.8
模型: GPT-5.4 (单模型, Zenmux)
国内模型: Kimi-K2.6 / Qwen3.6-Plus / GLM-5.1 (DashScope 百炼)
Temperature: 0.3
策略: AI 初筛 + 人工终审
```
...
- **可靠性阈值**：
  - High（高置信度）：std ≤ 5 分
  - Medium（中等置信度）：5 < std ≤ 8 分
  - Low（低置信度）：8 < std ≤ 12 分
  - Critical（需复核）：std > 12 分

#### 模型稳定性分析（2026-04-23）

基于 6 次真实评审测试的结果：

| 模型 | 供应商 | 时间稳定性 | 输出格式 | 综合评价 | 备注 |
|------|--------|-----------|----------|----------|------|
| **GPT-5.4** | Zenmux | ✅ 高（±4分） | ✅ 稳定 | **推荐** | 6个维度中5个稳定 |
| **Gemini 3.1 Pro** | Zenmux | ⚠️ 低（±35分） | ✅ 稳定 | 不推荐 | 评分波动大 |
| **Kimi-K2.6** | DashScope | ✅ 稳定 | ✅ 稳定 | 可用 | 通过百炼调用，JSON输出正常；Zenmux版解析失败率100% |
| **Qwen3.6-Plus** | DashScope | 待验证 | ✅ 稳定 | 可用 | 新增，需多维度收敛测试 |
| **GLM-5.1** | DashScope | 待验证 | ✅ 稳定 | 可用 | 新增，需多维度收敛测试 |

**供应商策略**：国外模型（GPT、Gemini、Claude）→ Zenmux；国内模型（Kimi、Qwen、GLM）→ DashScope 百炼

**关键发现**：
1. 模型随机性是影响标准差的主要因素
2. 单次测试不可靠，需要多次测试或使用稳定模型
3. GPT-5.4 单模型策略比多模型更稳定
4. 概念操作化有效降低标准差（现状洞察度 -41%）

**详细分析**：见 `docs/evaluation/std-analysis-summary-20260423.md`

#### 概念操作化定义（2026-04-23）

为提高 AI 评分的稳定性和可重复性，已将 6 个维度的模糊概念转化为可操作的检查清单：

**文档**：`docs/evaluation/concept-operationalization-v1.0.md`

**核心改进**：
1. **枢纽性** → 4 项可验证标准（影响范围广/争议焦点/理论基础/制度连锁）
2. **研究地图** → 3 项必备要素（到达点/未竟点/本文切入点）
3. **学术流派覆盖** → 可统计的数量指标（3个及以上/2个/1个）
4. **可操作的分析框架** → 4 项检查标准（概念界定/类型划分/分析步骤/后文调用）
5. **推理链条** → 4 项验证方法（前后衔接/不可颠倒/无跳跃/处理反驳）
6. **延展价值** → 4 项标准（未来研究方向/理论假设/方法论贡献/与正文关联）

**应用效果**：
- v2.8 框架应用概念操作化定义后，现状洞察度 std 从 15.6 降至 9.2（-41%）
...
- **推荐模型配置**：GPT-5.4 单模型（稳定性高，±4分）
- **不推荐模型**：Gemini 3.1 Pro（波动大，±35分）
- **新增可用模型**：Kimi-K2.6、Qwen3.6-Plus、GLM-5.1（均通过 DashScope 百炼调用，JSON输出稳定）
- **供应商策略**：国外模型→Zenmux，国内模型→DashScope 百炼
- **推荐框架**：v2.8（应用概念操作化定义）
- **Temperature 设置**：0.3（降低随机性）
- **AI 定位**：辅助而非替代人工，用于初筛（< 50分拒稿，50-70分人工复核，> 70分建议录用）
- **预期效果**：效率提升 50-70%，平均 std < 5 分，高置信度比例 > 80%

**详细分析**：`docs/evaluation/std-analysis-summary-20260423.md`
