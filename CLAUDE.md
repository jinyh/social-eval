...
| 版本 | 状态 | 说明 |
|------|------|------|
| v2.48 | **推荐使用**（收紧 rubric + 致命缺陷识别） | 问题创新性和分析框架增加致命缺陷前置检测；正负样本差距 19.3 分（v2.47 为 14.2）；快速验证测试通过（所有维度 std < 5） |
...
**v2.48 验证结果**（2026-05-13）：
- 测试样本：比例原则在民法上的适用及展开_郑晓剑.pdf
- 测试模型：qwen3.6-plus + glm-5.1
- 六维评分：问题创新性 91.0（std=1.4）、现状洞察度 90.0（std=0.0）、分析框架 92.5（std=3.5）、逻辑严密性 92.0（std=0.0）、前瞻延展性 82.5（std=3.5）
- 结论：✅ 所有维度高置信度，框架稳定可用
