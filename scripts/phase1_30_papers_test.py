...
def track_metrics(output_dir: Path, batch_num: int, all_scores: list,
    ...
    metrics = {
        ...
        "estimated_remaining_minutes": round((30 - total_papers) * (elapsed / total_papers) / 60, 1)
    }
...
def print_progress_report(batch_num: int, all_scores: list,
                         elapsed: float, total_papers: int,
                         std_over_8_count: int, critical_std_count: int):
    """每 10 篇输出一次进展报告"""
    print("\n" + "=" * 60)
    print(f"进展报告 - 批次 {batch_num} (已完成 {total_papers}/30 篇)")
    print("=" * 60)

    if all_scores:
        overall_avg = sum(all_scores) / len(all_scores)
        print(f"样本：{len(all_scores)} 篇（三大刊）")
        print(f"分数分布：")
        ...
        print(f"整体均值：{overall_avg:.1f} 分")

    print(f"模型一致性：")
    print(f"   - std > 8：{std_over_8_count} 个维度")
    print(f"   - std > 12：{critical_std_count} 个维度（严重分歧）")

    avg_time = elapsed / total_papers
    print(f"平均耗时：{avg_time:.1f} 秒/篇")
    print(f"已用时间：{elapsed/60:.1f} 分钟")

    remaining = (30 - total_papers) * avg_time
    print(f"预计剩余：{remaining/60:.1f} 分钟")
    print("=" * 60 + "\n")


async def main():
    parser = argparse.ArgumentParser(description="Phase 1: 30 篇论文验证测试（三大刊）")
    ...
    parser.add_argument("--output-dir", default="results/phase1-30-papers")
    parser.add_argument("--concurrency", type=int, default=3, help="论文级并发数（默认 3）")
    args = parser.parse_args()

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    manifest_path = Path("raw/phase1-30-papers/manifest.json")
    if not manifest_path.exists():
        print("错误：样本清单不存在")
        print("请先运行：.venv/bin/python scripts/phase1_sample_selection_30.py")
        return 1
    ...
    for i, paper in enumerate(samples, 1):
        out_path = output_dir / f"paper-{i:02d}.json"
        tasks.append(
    ...
    if all_scores:
        ...
        print(f"   - 70-80 分：{sum(1 for s in all_scores if 70 <= s < 80)} 篇")
        print(f"   - < 70 分：{sum(1 for s in all_scores if s < 70)} 篇")
        print(f"极端值比例: {extreme_ratio:.1f}% (目标 < 15%)")
    else:
        overall_avg = 0
        extreme_ratio = 0
    ...
    print(f"std > 12: {critical_std_ratio:.1f}% (目标 <= 10%)")
