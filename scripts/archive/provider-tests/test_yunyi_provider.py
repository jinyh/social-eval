#!/usr/bin/env python3
"""测试 YUNYI Provider 是否可以调用 GPT 5.5"""
import asyncio
import sys
...
async def test_yunyi_provider():
    """测试 YUNYI Provider 基本功能"""
    print("=" * 60)
    print("测试 YUNYI Provider - GPT 5.5")
    print("=" * 60)

    # 检查配置
    print("\n1. 检查配置...")
    if not settings.yunyi_api_key:
        print("❌ 错误：未配置 YUNYI_API_KEY")
        print("   请在 .env 文件中设置 YUNYI_API_KEY")
        return False

    print(f"✅ API Key: {settings.yunyi_api_key[:10]}...")
    print(f"✅ Base URL: {settings.yunyi_base_url}")

    # 创建 provider
    print("\n2. 创建 YunyiProvider...")
    provider = YunyiProvider("gpt-5.5")
...
async def main():
    ...
    # 测试 1: 基本 JSON 响应
    test1_passed = await test_yunyi_provider()
