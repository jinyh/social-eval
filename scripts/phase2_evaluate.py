#!/usr/bin/env python3
"""Phase 2 全量评审脚本（1830 篇，Round 1 + Round 2）

对清洗后的论文列表执行完整两轮评审流程：
- Round 1：4 模型并发 × 6 维度，复用 run_convergence_test()
- Round 2：交叉评审，复用 evaluate_dimension_cross_review()

支持断点续传、分批执行、内容审查容错。

用法：
    # 全量执行（R1 + R2）
    python scripts/phase2_evaluate.py \
        --paper-list results/phase2-paper-list-cleaned.json \
        --output-dir results/phase2-evaluation \
        --concurrency 5 \
        --rounds both

    # 分批执行
    python scripts/phase2_evaluate.py \
        --paper-range 1-300 --rounds both --concurrency 5

    # 只跑 Round 2（R1 结果已存在）
    python scripts/phase2_evaluate.py \
        --paper-range 1-300 --rounds r2 --concurrency 5
"""
...
async def main():
    parser = argparse.ArgumentParser(description="Phase 2 全量评审脚本")
    parser.add_argument(
        "--paper-list",
    ...
    parser.add_argument(
        "--output-dir",
        default="results/phase2-evaluation",
        help="输出目录",
    )
    parser.add_argument(
        "--concurrency",
